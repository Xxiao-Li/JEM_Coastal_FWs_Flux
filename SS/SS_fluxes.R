...
setwd("E:/00-food web functioning and stability/Data_manuscript/SS")
info <- read.csv("SS_fluxweb.csv" ,fileEncoding = "UTF-8-BOM")
info

diet <- read.csv("SS_diet.csv", header = TRUE,fileEncoding = "UTF-8-BOM")#, 
diet <- as.matrix(diet)
diet[is.na(diet)] <- 0
...
wightedtopology = function(fluxes, loop=FALSE){
  res<-c()
  # The flux matrix
  W.net<-as.matrix(diet) #fluxmatrix from fluxweb
...
flux.output <- wightedtopology(diet)
write.csv(flux.output, "linkwightedtopology.csv")


growth.rates = rep(NA, dim(mat.fluxes)[1])
growth.rates[colSums(mat.fluxes) == 0] = 0.5
